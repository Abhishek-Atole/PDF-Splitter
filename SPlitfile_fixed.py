...
def _rotate_page_obj(page, degrees):
    ...
    if not degrees or degrees % 360 == 0:
        return page
    try:
        # Some versions have rotate() that mutates; try it.
        if hasattr(page, "rotate"):
            try:
                page.rotate(degrees)
                return page
            except TypeError:
                # Some implementations expect no arg or different behavior; fall through
                pass
        # Newer PyPDF2 provides rotate_clockwise which returns a new page
        if hasattr(page, "rotate_clockwise"):
            return page.rotate_clockwise(degrees)
        if hasattr(page, "rotateCounterClockwise"):  # unlikely name
            return page.rotateCounterClockwise(degrees)
        # Last resort: try rotateCounterClockwise with complementary angle
        if hasattr(page, "rotate_counter_clockwise"):
            return page.rotate_counter_clockwise(360 - degrees)
    except Exception:
        # ignore and return original page
        return page
    return page
...
class PDFPreview:
    ...
    def load_pdf(self, pdf_path, status_callback=None):
        ...
        try:
            ...
            # Load all pages at once with DPI=72 for faster preview
...
class PDFToolApp:
    ...
    def build_image_tab(self):
        ...
        ttk.Button(proc_row, text="Process Images", command=self.run_image_process, style="Accent.TButton").pack(side="left", padx=6)
        ttk.Button(proc_row, text="Reset", command=self.image_reset, style="Outline.TButton").pack(side="left")
    ...
    def browse_file(self):
        ...
        self.file_display.config(text=os.path.basename(fp))
        try:
            reader = PdfReader(fp)
            self.total_pages.set(len(reader.pages))
            self.page_info.config(text=f"Pages: {len(reader.pages)}")
            # load preview (non-blocking not implemented)
    ...
    def on_range_key(self, event=None):
        ...
        if invalid:
            ...
            self.range_status.config(text="No ranges")
            self.split_btn.state(["disabled"])
        else:
            self.range_status.config(text="Ranges OK: " + ", ".join([f"{a}-{b}" for a,b in ranges]))
    ...
    def run_merge(self):
        ...
        try:
            ...
            self.merge_status.config(text=f"Merged to {out_path}")
    ...
    def run_image_process(self):
        ...
        self.image_status.config(text=f"Done. Success: {succ} Failed: {len(failed)}")
        if failed:
    ...
    def apply_theme(self):
        ...
        # apply to some common ttk styles
        self.style.configure("TFrame", background=pal['panel'])
        self.style.configure("TLabel", background=pal['panel'], foreground=pal['text'])
        self.style.configure("TButton", background=pal['accent'], foreground=pal['text'])
        self.style.configure("TEntry", fieldbackground=pal['card'], foreground=pal['text'])
